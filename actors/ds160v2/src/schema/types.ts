// Canonical types for the DS-160 applicant payload.
// Values arrive from upstream already in CEAC-ready format:
//   - Y/N for booleans
//   - Short codes for dropdowns (B1/B2, BRAZIL, SP, RCF, ...)
//   - ASCII-only strings (no diacritics)
//   - NA sentinels: 'DNA' | 'N/A' | 'n/a' | '' | null | undefined
...
// Input delivered to the Actor Run (parsed from INPUT_SCHEMA).
// Kept minimal — the template's stock INPUT_SCHEMA (startUrls/maxRequestsPerCrawl) is untouched,
// but Actor.getInput() returns whatever JSON the orchestrator passes at runtime.
