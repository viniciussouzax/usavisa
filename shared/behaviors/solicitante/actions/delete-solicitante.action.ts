...
import { deleteSolicitante } from "@/shared/models/solicitante";
import { db } from "@/db";
...
export async function deleteSolicitanteAction(input: Input): Promise<Result> {
  const parsed = schema.safeParse(input);
  if (!parsed.success) return { error: "UID inválido" };
  ...
  await deleteSolicitante(parsed.data.uid);
  return { error: null };
}
