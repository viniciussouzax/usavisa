...
import { reactivateSolicitanteShareLink } from "@/shared/models/solicitante-share-link";
import { db } from "@/db";
...
export async function reactivateSolicitanteShareLinkAction(
  input: Input,
): Promise<Result> {
  ...
  await reactivateSolicitanteShareLink(parsed.data.token);
  return { error: null };
}
