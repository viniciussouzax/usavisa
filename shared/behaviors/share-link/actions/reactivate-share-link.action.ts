...
import { reactivateShareLink } from "@/shared/models/share-link";
import { db } from "@/db";
...
export async function reactivateShareLinkAction(input: Input): Promise<Result> {
  ...
  await reactivateShareLink(parsed.data.token);
  return { error: null };
}
